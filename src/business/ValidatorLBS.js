...
import {
  ...
  MAX_NAME_LENGTH,
  GROUP_FORMAT_DATE
} from '../objects/business/be/GroupBE'
...
export const checkNameGroupBE = groupBE => {
  ...
  if (!groupBE.name) {
    errors.push(
      new ErrorDO('name', 'group.name.required', 'Le nom est obligatoire.')
    )
  ...
  if (!groupBE.description) {
    ...
    errors.push(
      new ErrorDO(
        ...
        'La longueur de la descrition ne doit pas dépasser ' +
          MAX_DESCRIPTION_LENGTH +
          ' caractères.'
      )
    )
...
export const checkStatusGroupBE = groupBE => {
  ...
  if (!groupBE.status) {
    errors.push(
      new ErrorDO('status', 'group.status.unknown', 'Le statut est inconnu.')
    )
  } else if (!(groupBE.status in GroupStatusEnum)) {
    errors.push(
      new ErrorDO('status', 'group.status.invalid', 'Le statut est invalide.')
    )
  }
  return errors
}
...
export const checkSelectionModeGroupBE = groupBE => {
  ...
  if (!groupBE.selectionmode) {
    errors.push(
      new ErrorDO(
        'selectionmode',
        'group.selectionmode.unknown',
        'Le mode de sélection est inconnu.'
      )
    )
    ...
    errors.push(
      new ErrorDO(
        'selectionmode',
        'group.selectionmode.invalid',
        'Le mode de sélection est invalide.'
      )
    )
  }
  return errors
...
export const checkTypeGroupBE = groupBE => {
  ...
  if (!groupBE.type) {
    errors.push(
      new ErrorDO('type', 'group.type.unknown', 'Le type est inconnu.')
    )
  } else if (!(groupBE.type in GroupTypeEnum)) {
    errors.push(
      new ErrorDO('type', 'group.type.invalid', 'Le type est invalide.')
    )
  }
  return errors
}
...
export const checkProposedDateGroupBE = groupBE => {
  ...
  if (!groupBE.proposedDate) {
    errors.push(
      new ErrorDO(
        'proposedDate',
        'group.proposedDate.unknown',
        'La date proposée est inconnue.'
      )
    )
    ...
    errors.push(
      new ErrorDO(
        'proposedDate',
        'group.proposedDate.invalid',
        'La date proposée est invalide.'
      )
    )
  }
  return errors
...
export const checkOpenedDateGroupBE = groupBE => {
  ...
  if (groupBE.openedDate && !checkFormatDate(groupBE.openedDate)) {
    errors.push(
      new ErrorDO(
        'openedDate',
        'group.openedDate.invalid',
        "La date d'ouverture est invalide."
      )
    )
  }
  return errors
...
export const checkClosedDateGroupBE = groupBE => {
  ...
  if (groupBE.closedDate && !checkFormatDate(groupBE.closedDate)) {
    errors.push(
      new ErrorDO(
        'closedDate',
        'group.closedDate.invalid',
        'La date de fermeture est invalide.'
      )
    )
  }
  return errors
